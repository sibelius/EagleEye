...
def train_classifier(filename_tweets, filename_labels, filename_classifier):
...
#    filename_labels = 'data/train_tweets_labels.txt'
    ...
    pickle.dump(pipeline, open(filename_classifier,'w'))
...
if __name__ == '__main__':
    if len(sys.argv) == 4:
        filename_tweets = sys.argv[1]
        filename_labels = sys.argv[2]
        filename_classifier = sys.argv[3]
        train_classifier(filename_tweets, filename_labels, filename_classifier)
    else:
        print("Usage: %s tweets labels classifier_name" % (sys.argv[0]))
